<?php
namespace phptests\Common\Database;
...
class PdoAdapter implements DatabaseAdapterInterface
{
        ...
        $cursorOffset = null)
    {
        ...
        try {
            return $this->getStatement()->fetch($fetchStyle, $cursorOrientation,
                $cursorOffset);
        } catch (PDOException $e) {
            throw new RunTimeException($e->getMessage());
        }
    ...
    public function fetchAll($fetchStyle = null, $column = 0)
    {
        ...
        try {
            return $fetchStyle === PDO::FETCH_COLUMN ? $this->getStatement()->fetchAll(
                $fetchStyle, $column) : $this->getStatement()->fetchAll(
                $fetchStyle);
        } catch (PDOException $e) {
            throw new RunTimeException($e->getMessage());
        }
    ...
    public function select($table, array $bind = array(), $boolOperator = "AND")
    {
        ...
        $sql = "SELECT * FROM " . $table . (($bind) ? " WHERE " .
